...
fname = "magnetoconductivity_EF=+8.01636_theta=0.npz"
...
plot_tensor(conductivity_tot, errorbar_tot, f"magnetoconductivity_EF={EF:.5f}_{B_dir_str}", "sigma")
rho = np.linalg.inv(conductivity_tot)
plot_tensor(rho, None, f"magnetoresistivity_EF={EF:+.5f}_{B_dir_str}", "rho")
...
plt.title(f"Magnetoresistance, EF={EF:.2f} eV, {B_dir_str}")
plt.legend()
plt.grid()
plt.savefig(f"MR_EF={EF:+.5f}_{B_dir_str}.png", dpi=300)
