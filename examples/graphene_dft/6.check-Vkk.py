...
wannier_centers_red = wandata.wannier_centers_cart @ np.linalg.inv(
    wandata.chk.real_lattice)
...
print(
    f"Max difference between Vkk on kpoints and Vkk on mp grid is {diffmax:.2e} eV")


kpt_red_left = np.array([[0, 0, 0.]])

nk = 40
kpt_red_right = np.array([[kx, ky, 0] for kx in np.linspace(
    0, 1, nk) for ky in np.linspace(0, 1, nk)])

kpt_cart_left = kpt_red_left @ wandata.chk.recip_lattice
kpt_cart_right = kpt_red_right @ wandata.chk.recip_lattice

Vkk_new = scatter.get_on_kpoints(
    kpt_red_left=kpt_red_left, kpt_red_right=kpt_red_right)

fig, ax = plt.subplots(4, 4, figsize=(12, 12))
for i in range(4):
    for j in range(4):
        ax[i, j].scatter(kpt_cart_right[:, 0], kpt_cart_right[:,
                         1], c=Vkk_new[0, :, i, j].real, s=5)
        ax[i, j].set_title(f"Vkk[{i},{j}]")
        ax[i, j].scatter(kpt_cart_left[:, 0],
                         kpt_cart_left[:, 1], c='red', s=50, marker='x')
        # colorbar
