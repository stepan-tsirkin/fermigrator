...
def get_contours_and_WFs(system,
    ...
    assert np.all(system.periodic == (True, True, False)
                  ), "system should be 2D periodic"
    energies_grid = None
    rec_lattice = None
    ...
    if energies_grid is None:
        ...
        if save_dir is not None:
            np.savez(os.path.join(save_dir, "energies_grid.npz"),
                     energies_grid=energies_grid, rec_lattice=rec_lattice)
        ...
        for i, e in enumerate(Efermi_list):
            ...
            if save_dir is not None:
                np.savez(os.path.join(
                    save_dir, f"contour_ib{ib}_EF={np.round(e, 5):.5f}.npz"), **dic_loc)
    if return_dict:
        return contours
